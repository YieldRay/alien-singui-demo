...
type BaseMime = `${string}/${string}`;

// Reactive value: can be a plain value or a getter/signal (both are () => T)
export type MaybeReactive<T> = T | (() => T);

// Extract event handlers (onXXX keys) from an attributes interface
type EventHandlerKeys<T> = {
  [K in keyof T]: K extends `on${string}` ? K : never;
}[keyof T];

// Non-event attribute keys
export type AttrKeys<T> = Exclude<keyof T, EventHandlerKeys<T>>;

// Make attribute values optionally reactive (exclude event handlers)
export type ReactiveAttrs<T> = {
  [K in AttrKeys<T>]?: T[K] extends infer V ? MaybeReactive<V> : never;
} & {
  [K in EventHandlerKeys<T>]?: T[K];
} & {
  [key: string]: unknown;
};

// Prop proxy: each property accepts plain value or reactive
type ReactiveProp<E> = {
  [K in keyof E]: MaybeReactive<E[K]>;
};

// Tools passed to the builder callback, strongly typed per element
export interface TagTools<E extends HTMLElement> {
  el: E;
  ...
  prop: ReactiveProp<E>;
  attr: {
    [K in AttrKeys<JSX.HTMLAttributes>]?: JSX.HTMLAttributes[K];
  } & Record<string, any>;
}

// Map from IntrinsicElements key to the corresponding HTMLElement type
type ElementTypeFor<K extends keyof IntrinsicElements> =
  K extends keyof HTMLElementTagNameMap ? HTMLElementTagNameMap[K] : HTMLElement;

// The tag proxy type: for each known element, provides strongly-typed overloads
export type TagProxy = {
  [K in keyof IntrinsicElements]: ((
    attrs: ReactiveAttrs<IntrinsicElements[K]>,
    builder?: (tools: TagTools<ElementTypeFor<K>>) => void,
  ) => ElementTypeFor<K>) &
    ((
      builder?: (tools: TagTools<ElementTypeFor<K>>) => void,
    ) => ElementTypeFor<K>);
  ...
  [K: string]: ((
    attrs: ReactiveAttrs<JSX.HTMLAttributes>,
    builder?: (tools: TagTools<HTMLElement>) => void,
  ) => HTMLElement) &
    ((
      builder?: (tools: TagTools<HTMLElement>) => void,
    ) => HTMLElement);
...
export declare namespace JSX {
    ...
    export interface HTMLAttributes extends JSXAttributes, EventAttributes {
